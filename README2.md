# 교실 PC 간 네트워크 속도 측정 도구 (iPerf3 기반)

교실의 **HP V1905-24 스위치**로 연결된 2대의 PC 간 실제 전송 속도를 측정하는 Python 스크립트입니다.
측정 엔진으로는 업계 표준 벤치마크 도구인 **iPerf3**를 사용합니다.

## 목차
- [측정 개념](#측정-개념)
- [교실 네트워크 구성](#교실-네트워크-구성)
- [사전 준비](#사전-준비)
- [설치 (한 번만)](#설치-한-번만)
- [실행 방법](#실행-방법)
- [옵션 설명](#옵션-설명)
...
교실에서는 **HP V1905-24**(JD990A) 스마트 스위치를 사용합니다.

| 항목 | HP V1905-24 사양 |
|------|------------------|
| 포트 | 24 × **10/100 Mbps** (Fast Ethernet) |
| 추가 포트 | 2 × 10/100/1000 Mbps 기가비트 (dual-personality) + 콘솔 1 |
| 처리량 | 6.6 million pps |
| 스위칭 용량 | 8.8 Gbps |
| 방식 | Layer 2 스마트 관리형 (웹 관리, 전이중 지원) |

- 이 장비는 **허브가 아닌 스위치**이므로 반이중 문제는 없습니다.
- 다만 24개 일반 포트는 **100Mbps까지만** 지원하므로, 일반 포트에 연결된 PC끼리는 물리적 한계가 **약 100 Mbps**입니다.
- **기가비트(900Mbps+) 측정**을 하려면 두 PC를 스위치 맨 끝의 **기가비트 포트(10/100/1000) 2곳**에 각각 연결하세요.
...
- 같은 교실의 **HP V1905-24 스위치** 아래에 연결돼 있으면 대부분 같습니다.
...
### 교실 스위치(HP V1905-24) 사용 시
- 교실의 HP V1905-24는 **스위치**이므로 반이중(half-duplex) 문제가 없고, 각 PC는 전용 대역폭을 사용합니다.
- **일반 포트(24개, 10/100)에 연결된 경우** 물리적 한계가 **~100 Mbps**입니다. 따라서 다운로드/업로드가 90~99 Mbps 수준으로 나오는 것은 정상입니다.
- **기가비트로 측정**하려면 두 PC를 스위치의 **기가비트 포트(10/100/1000, 2개)** 에 각각 연결해야 합니다.
- 같은 교실에서 다른 PC들이 동시에 사용 중이면 측정값이 낮아질 수 있습니다.

### 측정 시 유의점
1. **링크 속도 확인**: 측정 전 `ipconfig /all`에서 연결 속도가 `100 Mbps`인지 `1.0 Gbps`인지 확인하세요.
   - `100 Mbps`로 표시되면 스위치의 10/100 일반 포트에 연결된 것입니다.
   - `1.0 Gbps`로 표시되면 기가비트 포트에 연결된 것입니다.
2. 네트워크를 사용하는 다른 프로그램(온라인 수업, 스트리밍, 다운로드 등)을 종료하세요.
3. 와이파이보다 **유선(랜케이블)이 더 안정적**입니다.
4. Windows Defender 등 보안 프로그램은 속도에 영향을 줄 수 있습니다.
5. 서버 모드 시작 후 몇 초 후 클라이언트를 실행하세요.
6. 반드시 **한 방향씩** 순차 측정합니다. (이 도구는 기본적으로 방향별 순차 측정)
...
### 교실(HP V1905-24) 기준 정상 범위

| 연결 포트 | 정상 측정 범위 |
|-----------|----------------|
| 일반 포트 (10/100) | **90 ~ 99 Mbps** |
| 기가비트 포트 (10/100/1000) | **900 ~ 990 Mbps** |
...
+-----------------------------+---------------+---------------+---------------+-------------+-------------+
| TCP download (server->client)|      941.20 |      912.45 |      980.30 |             - |             - |
| TCP upload   (client->server)|      905.38 |      880.12 |      933.77 |             - |             - |
| UDP upload   (client->server)|       99.87 |       99.01 |       100.02 |         0.512 |          0.01 |
+-----------------------------+---------------+---------------+---------------+-------------+-------------+
```
...
- **일반 포트 연결 시 90~99 Mbps** 수준이 정상입니다 (100Mbps 물리적 한계).
- **기가비트 포트 연결 시 900~990 Mbps** 수준이 정상입니다.
- 값이 현저히 낮으면(예: 10Mbps대) 케이블 불량, 포트 불량, 다른 PC의 동시 사용, 드라이버 설정을 점검하세요.
...
| 결과가 낮게 나옴 | 다른 PC 동시 사용 종료, 케이블/포트 교체, 기가비트 포트 연결 확인 |
| 90~99 Mbps만 나옴 | 정상입니다. 일반 포트(10/100) 연결은 100Mbps가 물리적 한계 |
